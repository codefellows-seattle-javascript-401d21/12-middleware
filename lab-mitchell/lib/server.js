...
app.use('/{0,}', (req, res) => { //catchall for anything, what you're asking for doesn't exist in any of my route files so i create an error path err 404 and hand that over to error handler
  ...
  return errorHandler(err, res); //this is the one-off case in the drawing in notes for today
}); 
...
server.start = (port, callback) => { //can cache return value of server.start if we need it somewhere
  if(server.isOn) return callback(new Error('Server already running, cannot start server'));
  server.isOn = true;
...
server.stop = (callback) => { //callback whatever you want it to be
