...
describe('error-handler', function() {
  ...
  this.res = {status: function(stat){this.statusCode = stat
    return this
  ...
  it('should respond with a status of 400', () => {
    let error = errorHandler(this.validation, this.res)
    expect(error.statusCode).toEqual(400)
  })
  it('should respond with a status of 404', () => {
    let error = errorHandler(this.enoent, this.res)
    expect(error.statusCode).toEqual(404)
  })
  it('should respond with a status of 404', () => {
    let error = errorHandler(this.pathError, this.res)
    expect(error.statusCode).toEqual(404)
  })
  it('should respond with a status of 500', () => {
    let error = errorHandler(this.fail, this.res)
